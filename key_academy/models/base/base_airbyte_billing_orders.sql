...
unnest_data AS (
  ...
  LEFT JOIN UNNEST(JSON_QUERY_ARRAY(airbyte_data, '$.allowedPaymentMethods.paymentMethods')) AS payment_method
  LEFT JOIN UNNEST(JSON_QUERY_ARRAY(airbyte_data, '$.allowedPaymentMethods.additionalInformation.RiskAssessments')) AS risk_assessment
)
